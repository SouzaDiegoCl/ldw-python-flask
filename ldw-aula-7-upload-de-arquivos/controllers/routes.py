...
import urllib
import json
#Biblioteca que cria um identificador unico para algo
import uuid
...
def init_app(app):
    # Função de middleware para verificar a autenticação de usuário
    @app.before_request
    def check_auth():
        ...
        # Se a rota não estiver na lista branca
        # Veriica se o usuário está logado
    ...
    def login():
        if request.method == 'POST':
            ...
            #Usuario existe e senha correta?
            if user and check_password_hash(user.password, password):
                ...
                flash(f('Falha no login, Verifique o nome de usuario e senha', 'danger'))
                return redirect(url_for('login'))
    ...
    def cadastro():
        if request.method == 'POST':
            ...
            #Verificando de o usuario já existe
            user = Usuario.query.filter_by(username=username).first()
            #Se existir user
            if user: 
    ...
    def galeria():
        ...
        if request.method == 'POST':
            ...
            #Enviando o nome do arquivo para a função para verifica se o tpo de arquivo é permitido
