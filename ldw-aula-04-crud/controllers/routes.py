...
def init_app(app):
    ...
    def estoque(id=None):
        ...
        if request.method == 'POST':
            ...
            #Captura o valor de current page passado pela URL(query parameter)
            currPage = request.args.get('page', 1, type=int)
            #Qtd de itens por página
            qtdPerPage = 1
            
            # query.all é método do SQL Alchemy para selecionar todos os registros
            # query.paginate é o método para filtrar os registros de acordo com o limite
            gamesEstoque = Game.query.paginate(page=currPage, per_page=qtdPerPage)
